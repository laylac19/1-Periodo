...
%%Obter todos os irmãos de um dos alunos:
%?- gerou(X, layla), gerou(X, Y), Y\= layla.

%%Obter os avós paternos de um dos alunos
%?- gerou(X, Y), gerou(Y, layla).

%Obter todas as duplas pai/filho da base;
%?- gerou(X, Y).

%Obter as cunhadas da mãe do aluno:
%?- gerou(X, marivaldo), gerou(X, Y), mulher(Y).

%Obter quem é homem na família do aluno:
%?- homem(X).
...
% Obter todas as duplas avó/neto providos da mãe do aluno:
